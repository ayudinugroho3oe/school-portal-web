...
export function apiError(error: unknown, id: string) {
  if (error instanceof ApplicationError) return NextResponse.json({ error: { code: error.code, message: error.message, details: error.details }, meta: { requestId: id } }, { status: error.status });
