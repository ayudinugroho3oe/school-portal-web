...
export default async function ClassProgramPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main className="bg-[#F8FAFC]">
    ...
    <section className="px-5 py-16 sm:px-6 lg:py-24"><div className="mx-auto max-w-4xl rounded-[32px] bg-[linear-gradient(135deg,#0F766E,#10B981)] px-6 py-10 text-center text-white shadow-[0_24px_60px_rgba(15,118,110,0.20)] sm:px-10 sm:py-12"><h2 className="text-3xl font-extrabold">Tertarik Bergabung di Program {program.name}?</h2><p className="mx-auto mt-4 max-w-2xl text-emerald-50">Lengkapi formulir PPDB atau pelajari informasi pendaftaran sebelum memilih kelompok kelas.</p><div className="mt-7 flex flex-col justify-center gap-3 sm:flex-row"><Link href={`/ppdb/register?program=${program.id}`} className="inline-flex min-h-12 items-center justify-center rounded-[18px] bg-white px-6 font-bold text-teal-800">Daftar Program {program.name}</Link><Link href="/ppdb" className="inline-flex min-h-12 items-center justify-center rounded-[18px] border border-white/50 px-6 font-bold text-white hover:bg-white/10">Lihat Informasi PPDB</Link></div></div></section>
  </main>;
}
